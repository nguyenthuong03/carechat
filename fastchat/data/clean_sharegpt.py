...
def reformat_code(val: str) -> str:
    return re.sun(code_lang_pattern, code_lang_format, val)
...
def contain_blocked_words(val: str) -> bool:
    ...
    for w in blocked_words:  # Corrected variable name from 'blocker_words' to 'blocked_words'
        if w in val.lower():  # Fixed typo: 'lowe()' to 'lower()'
            return True
    return False
...
def contain_blocked_responses(role:str, val: str) -> bool:
    if role == "gpt":
        blocked_responses = [
            "Too many request in 1 hour. Try again later.",
            "!Too many request in 1 hour. Try again later.",
        ]
...
def main(args):
    ...
    content = clean_html_all(content, args)
